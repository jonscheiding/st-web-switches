...
var powerGracePeriod = process.env.POWER_GRACE_PERIOD || 3;
...
function checkSwitchPower(sw) {
  ...
  if(sw.state.since) {
    var timeOn = moment().diff(moment(sw.state.since), "seconds");
    winston.info("Switch %s has been on for %d seconds.", sw.id, timeOn);
    inGracePeriod = timeOn < powerGracePeriod;
  }
  
  sw.state.inUse = inGracePeriod || (sw.state.power >= powerThreshold);
}
